function [y,phn] = FeedforwardCPE_bps(x,mn,bs,method,appML,iter)
...
for pol = 1:size(x,2)
    ...
    for p = 1:M
        ...
        D(:,p) = smooth(abs(xp(:,p)-xd(:,p)).^2,1);
    end
%     [~,idx] = min(D,[],2);
%     rot_phi = phi(idx);

    Lenx = size(x,1);
    indx = round(-bs/2):round(bs/2);
    varD = var(D(indx(indx+1>0)+1,:),1);
    [~,idx1] = min(varD);
    rot_phi(1) = phi(idx1);
    for n = 2:size(D,1)
        indy = mod((idx1-10:idx1+10)-1,90)+1;
        varD = var(D(indx((indx+n>0)&(indx+n<Lenx))+n,indy),1);
        [~,idx2] = min(varD);
        rot_phi(n) = phi(indy(idx2));
        idx1 = indy(idx2);
    end
